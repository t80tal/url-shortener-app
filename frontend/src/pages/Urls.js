...
const Urls = () => {
  ...
  const { modal, id } = useSelector(state => state.ui.isEditingUrl)
  ...
  const closeEditing = () => {
    dispatch(uiActions.closeEditingUrl())
  }
  ...
  return (
    <BasePage name="Urls">
      <Wrapper>
        ...
        {modal && <Modal onClose={closeEditing}>
          asdeasd
        </Modal>}
      </Wrapper>
    </BasePage>
...
const Editbutton = id => {
  ...
  const onEditUrlHandler = event => {
    dispatch(uiActions.setIsEditingUrl(id))
  }

  return (
    <SvgWrapper id={id} onClick={onEditUrlHandler}>
